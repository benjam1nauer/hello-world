...
!!    wie INTEGER und REAL(-Felder) oder Vektor4. Besser nur die einfachen verwenden.
...
!!  - Es gibt keinen Grund GOTO zu verwenden; Funktionen mit RETURN verlassen
!!  - Fehlerkenner werden eindeutig gesetzt und in der Fehler.properties Datei eingetragen
!!  - Kommentieren mit Doxygen, sollte im Wiki oder aehnlichem auf Quelltext verwiesen
...
CONTAINS
  ...
  SUBROUTINE subroutineName(ausgabe1, ausgabe2, eingabe1, eingabe2, ierr, optionaleEingabe)
    ...
    REAL(real_normal), INTENT(INOUT) :: ausgabe2
  ...
  FUNCTION funktionsName (eingabe1, eingabe2) RESULT(ergebnis)
    REAL(real_normal), INTENT(IN) :: eingabe1, eingabe2
    REAL(real_norm) :: ergebnis
